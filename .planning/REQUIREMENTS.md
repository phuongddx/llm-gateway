...
- [x] **OBSV-02**: A client exceeding its configured per-key rate limit receives HTTP 429 from the gateway without provider traffic being sent; limit configurable via `.env`
...
- [x] **DOCS-01**: `docs/project-overview-pdr.md` and `docs/project-roadmap.md` describe the current two-provider (Manifest + zai-coding) architecture; superseded provider/pricing requirements explicitly marked historical
- [x] **DOCS-02**: `docs/deployment-guide.md` documents the Docker/Compose deployment and current env surface (no stale per-provider key/base-URL tables); `docs/code-standards.md` file tree and "Adding a New Provider" recipe match the shipped providers (no pricing step)
