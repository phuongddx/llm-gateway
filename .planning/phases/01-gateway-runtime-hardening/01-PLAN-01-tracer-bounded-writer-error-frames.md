...
<task type="tracer" tdd="true">
  ...
  <files>analytics/writer.py, config.py, .env.example, main.py, routes/chat.py, tests/conftest.py, tests/test_analytics_writer.py</files>
  <read_first>
  ...
  - AGENTS.md — Code Conventions & Testing sections (binding)
  </read_first>
  <behavior>
  ...
  - grep -c "analytics_writer" main.py returns >= 3 (import + construction + app.state) and the shutdown block calls writer stop before db close (source order assertion: the stop call line number is less than the db close call line number in main.py).
