...
<task type="tracer" tdd="true">
  ...
  - AGENTS.md — Architecture & Data Flow steps 6-7, Code Conventions (Async bullet), Testing & QA fixtures list (binding; action step 8 updates these references: the three analytics-path ones to the bounded-queue + single-writer pattern, plus the step-7 generic SSE frame to the nested error object)
  </read_first>
  <behavior>
  ...
  - AGENTS.md instructions stay true post-cutover: grep -c "add_done_callback" AGENTS.md returns 0 (stale fire-and-forget pattern removed from step 6 and the Async bullet), grep -c "analytics_writer" AGENTS.md returns >= 3 (Architecture step 6 enqueue + analytics_writer fixture entry + client(analytics_db, analytics_writer) fixture), and grep -c '{"error": "Internal error processing request"}' AGENTS.md returns 0 while grep -c "Internal error processing request" AGENTS.md returns 1 (step 7 now documents the nested generic object; message string preserved exactly once).
