...
- [x] Two providers: Manifest (500+ model passthrough) and native GLM (Z.AI) — later superseded 2026-04-19 by the Manifest cutover and 2026-09-07 by the z.ai GLM Coding Plan re-route (see project-overview-pdr.md's Historical notation)
...
- [x] Model pricing table with cost calculation (removed 2026-04-19; `calculate_cost()` now stubs `0.0`, z.ai credit estimation tracks GLM usage instead — see `analytics/cost.py`)
...
|---------|-------------|----------|
| Docker image | Multi-stage Dockerfile with non-root user — Delivered | High |
| Docker Compose | Local dev and production compose configs — Delivered | High |
| CI/CD pipeline | GitHub Actions: lint, test, build, push image — Delivered | High |
| Prometheus metrics | Request count, latency histogram, error rate — Delivered | Medium |
| Health checks | Liveness (process) and readiness (provider connectivity) — Delivered | Medium |
| Rate limiting | Per-key request rate limits — Delivered | Medium |
| Error retry | Automatic retry with backoff on transient provider errors — Delivered | Medium |
...
- **Phase 3**: Docker-based deployment with CI/CD and monitoring (achieved)
